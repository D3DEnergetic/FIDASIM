FUNCTION east_equil,inputs,grid,det
; Returns the fields from an EFIT g eqdsk mapped onto the fidasim grid
; Also gets rho values along the detector lines of sight

	;; Get eqdsk
gfile=inputs.gfile
gfiletest=findfile(gfile)
if gfiletest eq '' then begin
  print, 'FATAL ERROR! gfile not found:', gfile
  equil={err:1}
  return,equil
endif else begin
