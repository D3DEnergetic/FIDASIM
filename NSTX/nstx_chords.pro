FUNCTION nstx_chords,shot,fida_diag,isource=isource,doplot=doplot
    ...
    ;;**********************************************************************
    ;; Vertial FIFA system, refer to fida_mario.pro in IDL FIDAsim4.0
    ...
    angle_transp_uvz=142.2/180.*!pi ;check angle_transp_uvz within nstx_beams.pro
    
    CXRSTA_nstx= [replicate(221.7,4),replicate(183.6,8)]
    nch_nstx_npa=n_elements(CXRSTA_nstx)
    CXYSTA_nstx=replicate(0.,nch_nstx_npa)
    CXZETA_nstx= [replicate(0.,4),   replicate(30.,8)]/180.*!pi+angle_transp_uvz 
    ...
    ;;npa_solid_angle_nstx=!pi*det_radius_nstx^2*npa_solid_angle_nstx
                  
    if n_elements(CXRSTA_nstx) ne n_elements(CXYSTA_nstx)  or $
       n_elements(CXRSTA_nstx) ne n_elements(CXZETA_nstx)  or $
       n_elements(CXRSTA_nstx) ne n_elements(CXTHETA_nstx) or $  
       n_elements(CXRSTA_nstx) ne n_elements(CXTHETA_nstx) or $     
       n_elements(CXRSTA_nstx) ne n_elements(det_radius_nstx) then begin 
        print,'Please check CXRSTA, CXYSTA,CXZETA,CXTHETA,and Rtan '+  $
              'in nstx_chords.pro'
        stop
    endif
    ...
    CXRSTA_tssnpa=replicate(188.0,5)                   ;! RADIUS FROM MACHINE CENTERLINE OF CX PIVOT
    CXYSTA_tssnpa=replicate(0.0,5)             ;! ELEVATION OF PIVOT ABOVE MIDPLANE
    CXZETA_tssnpa=replicate(61.7,5)/180*!pi+angle_transp_uvz 
                                                       ;! TOROIDAL LOCATION OF PIVOT IN DEGREES
    CXRTAN_tssnpa=[140., 130.,120.,110.,100.]          ;! CX RTAN (+CO, -CTR)
    CXTHEA_tssnpa=replicate(0.0,5)                 ;! ANGLE OF CHORD ABOVE MIDPLANE IN DEGREES
    CXYTAN_tssnpa=replicate(0.0,5)                     ;! HEIGHT ABOVE MIDPLANE OF TANGENCY RADIUS
    CXTHETA_tssnpa=replicate(0.0,5)  
    ...
    tssnpa_area=0.01d
    tssnpa_l=2.54d*3.0
    tssnpa_aperture=sqrt(tssnpa_area/!pi)
    
    det_radius_tssnpa=[replicate(sqrt(tssnpa_area/!pi),5)] 
    aperture_radius_tssnpa=[replicate(sqrt(tssnpa_area/!pi),5)]
    dcollimator_tssnpa=[replicate(sqrt(tssnpa_l),5)]     
    ...
    CXRSTA_rssnpa=replicate(203.0,5)                   ;! RADIUS FROM MACHINE CENTERLINE OF CX PIVOT
    CXYSTA_rssnpa=replicate(0.0,5)             ;! ELEVATION OF PIVOT ABOVE MIDPLANE
    CXZETA_rssnpa=replicate(-39.4,5)/180*!pi+angle_transp_uvz 
                                                       ;! TOROIDAL LOCATION OF PIVOT IN DEGREES
    CXRTAN_rssnpa=[90.,80.,70.,60.,50.]                ;! CX RTAN (+CO, -CTR)
    CXTHEA_rssnpa=replicate(0.0,5)                 ;! ANGLE OF CHORD ABOVE MIDPLANE IN DEGREES
    CXYTAN_rssnpa=replicate(0.0,5)                     ;! HEIGHT ABOVE MIDPLANE OF TANGENCY RADIUS
    CXTHETA_rssnpa=replicate(0.0,5)  
    ...
    rssnpa_area=0.01d
    rssnpa_l=2.54d*3.0
    rssnpa_aperture=sqrt(rssnpa_area/!pi)
    
    det_radius_rssnpa=[replicate(sqrt(rssnpa_area/!pi),5)] 
    aperture_radius_rssnpa=[replicate(sqrt(rssnpa_area/!pi),5)]
    dcollimator_rssnpa=[replicate(sqrt(rssnpa_l),5)]     
    ...
    CXZETA_pssnpa=replicate(-88.6,5)/180*!pi+angle_transp_uvz 
                                                       ;! TOROIDAL LOCATION OF PIVOT IN DEGREES
    ...
    CXZETA_nstxu_npa=replicate(8.0,5)/180*!pi+angle_transp_uvz 
                                                       ;! TOROIDAL LOCATION OF PIVOT IN DEGREES
    ...
    for i=0,n_elements(fida_diag)-1 do begin
        CASE strupcase(fida_diag[i]) OF
            ...
            'ACTIVE_VFIDA': begin
                ;coordinate of intersection of this fiber view with midplane
                xlens_active_vfida=replicate(detp_active_vfida[0],n_elements(xmid_active_vfida))
                ylens_active_vfida=replicate(detp_active_vfida[1],n_elements(xmid_active_vfida)) 
                zlens_active_vfida=replicate(detp_active_vfida[2],n_elements(xmid_active_vfida))    
            ...
            'PASSIVE_VFIDA': begin
                ;coordinate of intersection of this fiber view with midplane
                xlens_passive_vfida=replicate(detp_passive_vfida[0],n_elements(xmid_passive_vfida))
                ylens_passive_vfida=replicate(detp_passive_vfida[1],n_elements(xmid_passive_vfida)) 
                zlens_passive_vfida=replicate(detp_passive_vfida[2],n_elements(xmid_passive_vfida)) 
    ...
    if keyword_set(doplot) then begin
        ...
        for ich=0,fida.nchan-1 do begin
            x=fida.xlens[ich]+(fida.xlos[ich]-fida.xlens[ich])*findgen(npt)/(npt-1.)
            y=fida.ylens[ich]+(fida.ylos[ich]-fida.ylens[ich])*findgen(npt)/(npt-1.)
            oplot,x,y,color=!red
            oplot,fida.xlens,fida.ylens,color=!red,psym=1
        endfor
    endif
