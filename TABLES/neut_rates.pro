...
@all_tables.pro
PRO neut_rates,low_res=low_res,plot=plot,ps=ps
...
;; and by unbundeling the total cross sections from janev !!
  ...
  maxeb=600. 
  neb=6001
  if keyword_set(low_res) then neb=601
  ...
  for n=0,nmax-1 do begin
     for m=0,nmax-1 do begin
        for ie=0L,neb-1 do begin 
          ; if n eq 0 and m eq 2 then writeu, lun, float(0.) else $
              writeu, lun, float(sigma[m,n,ie])
        endfor
     endfor
  endfor
  close,lun
