pro plot_npa,ps=ps,path=path
  ...
  headsize=mean(det.npa.headsize)
  ...
  xran = [100,350]
  yran = [-175,25]
  contour,total(dens,3),fidasim.coords.xxc,fidasim.coords.yyc $
          ,c_colors=(dindgen(20)+1.)*11,nlevels=20 $
          ,xran=xran,yran=yran,/isotropic,xtit='X [cm]',ytit='Y [cm]'
  for ichan=0,det.nchan-1 do oplot,[det.xhead[ichan],det.xlos[ichan]] $
                                   ,[det.yhead[ichan],det.ylos[ichan]]
...
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  
  ...
  for idet=0,ndet-1 do begin ;; loop over detectors
     for i=0L,nnpa-1 do begin  ;; loop over markers that reach the detector
        ...
        dummy=min(abs(fidasim.coords.xxc-npaipos[i,0]),xind)
        dummy=min(abs(fidasim.coords.yyc-npaipos[i,1]),yind)
        dummy=min(abs(fidasim.coords.zzc-npaipos[i,2]),zind)
        bvec=reform(fidasim.plasma.b[xind,yind,zind,*])
        ;;normalize
