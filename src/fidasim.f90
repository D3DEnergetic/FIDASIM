...
subroutine gyro_step(vi, fields, r_gyro)
    ...
    real(Float64), dimension(3) :: vxB, uvw, B_rzt, grad_B, rg_cyl
    ...
    if(inputs%no_flr.eq.0) then
        ...
        inv_rg_r = 1/sqrt(uvw(1)**2 + uvw(2)**2)
        B_rzt(1) = fields%br/fields%b_abs
        B_rzt(2) = fields%bz/fields%b_abs
        B_rzt(3) = fields%bt/fields%b_abs
        vxb(1) = (inv_rg_r*fields%dbz_dphi - fields%dbt_dz)/fields%b_abs
        vxb(2) = (inv_rg_r*fields%bt + fields%dbt_dr - inv_rg_r*fields%dbr_dphi)/fields%b_abs
        vxb(3) = (fields%dbr_dz - fields%dbz_dr)/fields%b_abs
        term1 = vpar*one_over_omega*dot_product(B_rzt,vxb)
