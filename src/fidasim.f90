...
subroutine read_cfpd
    ...
    !! Calculate dl from sightline data
...
subroutine read_tables
    ...
    if(inputs%calc_neutron.ge.1.or.inputs%calc_neut_spec.ge.1.or.inputs%calc_cfpd.eq.1) then
...
subroutine get_bt_rate(plasma, eb, rate, branch, reaction)
    ...
    integer, intent(in), optional   :: reaction
        !+ Reaction type: 1 = D-D, 2 = D-He3. Default: based on calc_cfpd

    integer :: err_status, neb, nt, ebi, tii, is, ib, rxn
...
subroutine get_pgyro(fields,E3,E1,pitch,plasma,v3_xyz,pgyro,gam0,mass_amu,gammaplus,gammaminus)
    ...
    ccminus= 1/mu3 * (q - (mu1**2-mu1)*norm_v1**2 + (mu2**2-mu2)*norm2(vrot)**2 - 2*mu1*mu2*(vpar*vb-vperp*va))
    ccplus = 1/mu3 * (q - (mu1**2-mu1)*norm_v1**2 + (mu2**2-mu2)*norm2(vrot)**2 - 2*mu1*mu2*(vpar*vb+vperp*va))
    ...
    v3maxmag = sqrt(2*E3max*JkeV/m3)
    v3minmag = sqrt(2*E3min*JkeV/m3)

    if (E3plus.gt.E3max) then !'Case 3'
        norm_v3 = v3maxmag
        rhs = (1+mu3)*norm_v3 - q/(mu3*norm_v3) - (2*mu1*vpar+2*mu2*vb)*cos(phip) - 2*mu2*va*sin(phip) &
              + (mu1**2-mu1)*norm_v1**2/(mu3*norm_v3) + (mu2**2-mu2)*norm2(vrot)**2/(mu3*norm_v3) + &
                    ...
                    pitch_loop: do ip = 1, fbm%npitch
                        ...
                        energy_loop: do ie =1, fbm%nenergy
                            ...
                            gyro_loop: do igamma=1, ngamma
                                ...
                                call get_bt_rate(plasma, erel, rate, branch=2, reaction=1)
                                if(inputs%calc_neutron.ge.2) then
        ...
        channel_loop: do ichan=1, nc_chords%nchan
            ...
            if(inputs%dist_type.eq.2) then
                ...
                gyro_loop: do igamma=1,ngamma
                    ...
                    !! Get neutron production rate
                    call get_bt_rate(plasma, eb, flux, branch=2, reaction=1)
                ...
                loop_along_track: do i=1,ntrack
                 ...
                 !!!factor = domega*beam_grid%dv/ngamma
                    ...
                    gyro_loop: do igamma=1, ngamma
                        ...
                        call get_bt_rate(plasma,erel,flux,branch=2,reaction=1)
