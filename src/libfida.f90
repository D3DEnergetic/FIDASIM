...
type, extends(BirthParticle) :: SinkParticle
    ...
    !! >>>>>>>>>>>> [JFCM 2025-01-28] >>>>>>>>>>>>
    real(FLoat64) :: denf4d = 0.d0
        !+ Value of ion f4d at the given energy and pitch [p/cm^3 (s/cm)^3]
    real(FLoat64) :: denf4d_per_marker = 0.d0
        !+ Value of ion f4d at the given energy and pitch PER MARKER [p/cm^3 (s/cm)^3]
    !! <<<<<<<<<<<< [JFCM 2025-01-28] <<<<<<<<<<<<<
...
subroutine write_nlaunch(papprox,nlaunch,suffix)
  ...
  ! TODO: add new argument "gen" so we can potentially store all HALO generation nlaunch data
...
subroutine write_sink_profile(gen)
  ...
  allocate(inds(3,npart))
  allocate(neut_types(npart))
  allocate(atomic_mass(npart))
  allocate(denf4d(npart),denf4d_per_marker(npart))
  ...
  do i=start_index, end_index
      ...
      energy(i) = part%energy
      weight(i) = part%weight
      atomic_mass(i) = part%atomic_mass
      denf4d(i) = part%denf4d
      denf4d_per_marker(i) = part%denf4d_per_marker
  ...
  call parallel_sum(inds)
  call parallel_sum(neut_types)
  call parallel_sum(atomic_mass)
  call parallel_sum(denf4d)
  call parallel_sum(denf4d_per_marker)
  if(my_rank().ne.0) do_write = .False.
#endif
  ...
  if(do_write) then
      ...
      call h5ltmake_compressed_dataset_double_f(fid,"/weight", 1, dim2(2:2), weight, error)
      call h5ltmake_compressed_dataset_int_f(fid,"/type", 1, dim2(2:2), neut_types, error)
      call h5ltmake_compressed_dataset_double_f(fid,"/atomic_mass", 1, dim2(2:2), atomic_mass, error)
      call h5ltmake_compressed_dataset_double_f(fid,"/denf4d", 1, dim2(2:2), denf4d, error)
      call h5ltmake_compressed_dataset_double_f(fid,"/denf4d_per_marker", 1, dim2(2:2), denf4d_per_marker, error)
      ...
      call h5ltset_attribute_string_f(fid, "/dens", "units", &
           "Ions/(s*cm^3)", error)
      call h5ltset_attribute_string_f(fid, "/ri_gc", "description", &
           "Ion guiding-center birth position in R-Z-Phi: ri_gc([r,z,phi],particle)", error)
      call h5ltset_attribute_string_f(fid, "/ri_gc", "units", "cm, radians", error)
      call h5ltset_attribute_string_f(fid, "/ri", "description", &
           "Ion birth position in R-Z-Phi: ri([r,z,phi],particle)", error)
      call h5ltset_attribute_string_f(fid, "/ri", "units", "cm, radians", error)
      call h5ltset_attribute_string_f(fid, "/vi", "description", &
           "Ion birth velocity in R-Z-Phi: vi([r,z,phi],particle)", error)
      call h5ltset_attribute_string_f(fid, "/vi", "units", "cm/s", error)
      call h5ltset_attribute_string_f(fid, "/energy", "description", &
           "Ion birth energy: energy(particle)", error)
      call h5ltset_attribute_string_f(fid, "/energy", "units", "keV", error)
      call h5ltset_attribute_string_f(fid, "/weight", "description", &
           "Ion sink weight: weight(particle)", error)
      call h5ltset_attribute_string_f(fid, "/weight", "units", "fast-ions/s", error)
      call h5ltset_attribute_string_f(fid, "/pitch", "description", &
      ...
      !! >>>>>>>>>>>>>> [JFCM 2025-01-28] >>>>>>>>>>>>>>>
      call h5ltset_attribute_string_f(fid, "/denf4d", "description", &
           "Value of ion distribution function in 4D phase space at ri and vi position", error)
      call h5ltset_attribute_string_f(fid, "/denf4d", "units", "[ions/cm^3 (s/cm)^3]", error)
      call h5ltset_attribute_string_f(fid, "/denf4d_per_marker", "description", &
           "Value of ion distribution function in 4D phase space at ri and vi position PER MARKER", error)
      call h5ltset_attribute_string_f(fid, "/denf4d_per_marker", "units", "[ions/cm^3 (s/cm)^3]", error)
      !! <<<<<<<<<<<<<< [JFCM 2025-01-28] <<<<<<<<<<<<<<<
      ...
      call h5ltset_attribute_string_f(fid, "/", "version", version, error)
      call h5ltset_attribute_string_f(fid, "/", "description", &
           "Ion sink density and particles calculated by FIDASIM", error)
...
subroutine track_to_wall(rin,vin,tracks,ntrack,absorbed)
    ...
    integer(Int32), intent(out)                      :: ntrack
        !+ Number of cells that a particle crosses
    logical, intent(out) :: absorbed
        !+ logical flag indicating if particle was absorbed at a surface

    real(Float64), dimension(3) :: rn, vn, ri_cell, dr, dt_arr
    integer, dimension(3) :: ind, sgn, gdims
    integer :: cc, mind
    real(Float64) :: T_wall, v_thermal, p_specular, p_absorb, p_thermal, p_reflect
    real(Float64), dimension(1) :: randomu
    logical :: in_grid, collision_in_cell, surface_in_voxel, has_surface_collision
    ...
    ! -------------------------
    do while (in_grid .and. .not. absorbed)

      !! RESET decision variables:
      ! --------------------------
      collision_in_cell = .FALSE.
      surface_in_voxel = .FALSE.
      ...
      dr = beam_grid%dr*sgn
      dt_arr = huge(1.d0)
      where (vn /= 0.d0) ! Protect against vn(i) == 0
          dt_arr = abs(((ri_cell + 0.5d0*dr) - rn) / vn)
      end where
      mind = minloc(dt_arr,1)
      dT_next = dt_arr(mind)
      ...
      if (surface_in_voxel) then
        ...
        if (collision%n_events > 0) then
          ...
          ! GET collision event (choose 1st and closest root):
          event = collision%event(1)
          surface_id = event%surface_id
          region_id  = event%region_id

          !! CHECK surface id:
          if (surface_id < 1 .or. surface_id > size(vessel%surface)) then
              write(*,*) "ERROR(track_to_wall): invalid surface_id = ", surface_id
              write(*,*) "Number of surfaces = ", size(vessel%surface)
              stop
          endif

          !! CHECK region allocation:
          if (.not. allocated(vessel%surface(surface_id)%region)) then
              write(*,*) "ERROR(track_to_wall): region array not allocated"
          ...
          !! CHECK region id:
          if (region_id < 1 .or. &
              region_id > size(vessel%surface(surface_id)%region)) then

              write(*,*) "ERROR(track_to_wall): invalid region_id = ", region_id
              write(*,*) "surface_id = ", surface_id
              write(*,*) "Number of regions = ", &
                  size(vessel%surface(surface_id)%region)
      ...
      end if ! surface_in_voxel

      !! COMPARE time steps and SET collision_in_cell flag:
      ! ------------------------------------------
      if (has_surface_collision .and. dT_coll < dT_next) then
        collision_in_cell = .TRUE.
        dT = dT_coll
      else
        collision_in_cell = .FALSE.
        dT = dT_next
      ...
      !! CHECK collision_in_cell flag and COMPUTE collision effects:
      ! ----------------------------------------------
      if (collision_in_cell) then ! UPDATE ray velocity

        ! At this point, opening events should already have been filtered out.
        ! Therefore this should only be a physical wall interaction.

        if (behavior_type == "opening") then
            write(*,*) "ERROR: opening reached collision_in_cell block unexpectedly"
            stop
        endif
...
subroutine store_sinks(ind, is, flux_per_marker)
    !+ Store sink particles/density in [[libfida:sink]]
    integer(Int32), dimension(3), intent(in) :: ind
        !+ [[libfida:beam_grid]] indices
    integer(Int32), intent(in) :: is
        !+ Ion species index
    real(Float64), intent(in) :: flux_per_marker
        !+ Particle flux [p/s /m3]

    !$OMP ATOMIC UPDATE
    sink%dens(is,ind(1),ind(2),ind(3))= &
     sink%dens(is,ind(1),ind(2),ind(3)) + flux_per_marker
    !$OMP END ATOMIC
...
subroutine ndmc
    ...
    !$OMP& states,dens,iflux,photons,neut_type,jj,ii,kk,ind,err,ri,ri_gc,r_gyro, absorbed)
    loop_over_markers: do ii=istart,inputs%n_nbi,istep
        energy_fractions: do neut_type=1,3
            ...
            absorbed = .FALSE.
            call track_to_wall(rnbi,vnbi,tracks,ntrack,absorbed)
            if (ntrack .eq. 0) then
...
subroutine store_sink_particle(ind,ri,vion,is,flux,denf4d,denf4d_per_marker,fields)
  !+ Records an ion sink particle into [[libfida::sink]]
  integer,dimension(3), intent(in) :: ind
      !+ 3D index of cell where sink originated (ri)
  real(Float64), dimension(3), intent(in) :: ri
      !+ Ion start position vector
  real(Float64), dimension(3), intent(in) :: vion
      !+ Ion velocity vector participating in the sink reaction
  integer(Int32), intent(in) :: is
      !+ Ion species index
  real(Float64), intent(in) :: flux
      !+ Ion flux per volumer PER MARKER
  real(Float64), intent(in) :: denf4d
      !+ Value of ion distribution function at ri, vion [ion/cm^3 (s/cm)^3]
  real(Float64), intent(in) :: denf4d_per_marker
      !+ Value of ion distribution function at ri, vion PER MARKER [ion/cm^3 (s/cm)^3]
  type(LocalEMFields), intent(in) :: fields
      !+ Field values at ri position
  ...
  sink%part(sink%cnt)%weight = flux*beam_grid%dv ! flux per marker [p/s]
  ...
  !! Ion pitch at GC position:
  sink%part(sink%cnt)%pitch = dot_product(fields%b_norm,vion/norm2(vion))

  !! Get position vector pointing from ri to GC position:
  !! input: vion, fields, thermal_mass(is)
  !! output: r_gyro (dim 3)
  !! r_gyro is a vector from actual particle position "ri" to GC position:
...
! >>> [JFCM, 2025-08-20] >>>
subroutine mc_sample_ion_f4d_gc(ind_gc,is,rgc,deni_gc,fields_gc,rp,vp,ind_p,denf4d,f4d_defined)
  !+ Samples an ion from the guiding center distribution function [[libfida::fbm]]
  !+ For regions beyond the LFCS, fbm is zero so we sample from thermal plasma in SOL
  !+ Sampling [[libfida::fbm]] or a thermal distribution in the SOL returns the particle
  !+ energy and pitch.
  !+ Fields are evaluated at the GC position and are used to convert energy and pitch
  !+ into particle position (with gyro offset) and particle velocity.
  ! TODO: How to consider the effect of rotation on the nonthermal f4d sampling
  ! TODO: Remove the use of denf4d, no longer needed

  integer, dimension(3), intent(in) :: ind_gc
    !+ [[libfida:beam_grid]] 3D index of Guiding-Center (GC) position
  integer, intent(in) :: is
    !+ Index for the neutral species to be used
  real(Float64), dimension(3), intent(out) :: rgc
    !+ Position vector of GC in [[libfida:beam_grid]] XYZ coordinates [cm]
  real(Float64), intent(out) :: deni_gc
    !+ Ion density of species "is" at the GC position [cm^-3]
  type(LocalEMFields), intent(out) :: fields_gc
    !+ Fields at the GC position
  real(Float64), dimension(3), intent(out) :: rp
    !+ Position vector of particle in [[libfida:beam_grid]] XYZ coordinates [cm]
  real(Float64), dimension(3), intent(out) :: vp
    !+ Velocity vector of particle in [[libfida:beam_grid]] XYZ coordinates [cm/s]
  integer, dimension(3), intent(out) :: ind_p
    !+ [[libfida:beam_grid]] 3D index of particle position rp
  real(Float64), intent(out) :: denf4d
    !+ value of ion distribution at phase space position rgc and velocity vn [ion cm^{-3} v^{-3}]
  ...
  integer, dimension(2,1) :: ep_ind
  real(Float64), dimension(1) :: randomu1
  real(Float64), dimension(3) :: randomu3
  type(LocalProfiles) :: plasma
  real(Float64) :: eb, ptch

  ! GET valid GC position in cell:
  inp = .FALSE.
  it = 0
  do while (.not. inp .and. it .le. max_it)
    ! Uniformly sample GC position in cell:
    call mc_beam_grid(ind_gc,rgc)

    ! CHECK if valid:
    call in_plasma(rgc,inp)

    ! UPDATE Counter:
    it = it + 1
  end do
  if (.not. inp) call get_position(ind_gc,rgc)

  ! GET fields at GC position:
  call get_fields(fields_gc,pos=rgc)

  ! CHECK
  if (.not.fields_gc%in_plasma) then
    write(*,*) "ERROR in mc_sample_ion_f4d_gc at get_fields"
    stop
  end if
  ...
  ! CALCULATE ion energy, pitch and GC density:
  if (use_f4d .and. f4d_defined) then
    ! Sample non-thermal ion:
    call randind(fbeam,ep_ind)
    call randu(randomu3)
    ...
    ! Sample thermal ion:
    call get_plasma(plasma,pos=rgc)
    call mc_halo(plasma,thermal_mass(is),vp)
    eb = (norm2(vp)**2)*thermal_mass(is)*v2_to_E_per_amu ! [keV]
    call randu(randomu1)
    ptch = 2*randomu1(1) - 1
    deni_gc = plasma%deni(is)
    denf4d = 0.d0 ! Not needed anymore
  ...
  ! Convert ion energy and pitch into ion particle position and velocity:
  call gyro_correction(fields_gc,eb,ptch,thermal_mass(is),rp,vp)

  ! GET indices of particle position:
  call get_indices(rp,ind_p)

end subroutine mc_sample_ion_f4d_gc
! <<< [JFCM, 2025-08-20] <<<

!! >>>>>>>>>>> [jfcm, 2024_11_22] >>>>>>>>>>>>
! needs to be deleted once replaced with "mc_sample_ion_f4d_gc" in non_thermal_beam_stopping options
subroutine mc_ion_velocity_f4d(ind,is,rn,vn,denf4d,f4d_defined)
  ...
  real(Float64) :: deni, ti, prefactor, vth2, vn2, exp_term
...
subroutine calculate_wall_source_process(ss,rr)
  ...
  !$OMP& starting_flux,absorbed,initial_flux,final_flux,plasma,photons, &
  !$OMP& weight)
  loop_over_markers: do ii=istart,nlaunch,istep
    ...
    absorbed = .FALSE.
    call track_to_wall(rp,vp,tracks,ntrack,absorbed)
...
subroutine calculate_dcx_process
  ...
  integer(Int64) :: idcx !! counter
  real(Float64), dimension(3) :: rp    !! Ion particle position vector
  real(Float64), dimension(3) :: vp  !! Ion particle velocity vector
  integer,dimension(3) :: ind, ind_p, ind_res
  integer,dimension(3) :: neut_types = [1,2,3]
  type(LocalProfiles) :: plasma
  ...
  real(Float64) :: photons, weight
  real(Float64) :: denf4d, denf4d_per_marker, deni_gc
  !! >>> [JFCM, 2025-07-04] >>>
  logical :: absorbed
  !! <<< [JFCM, 2025-07-04] <<<
  !! >>> [JFCM, 2025-07-01] >>>
  ...
  !$OMP& starting_flux,flux_per_marker,denf4d_per_marker,denf4d,absorbed, &
  ...
  loop_over_cells: do ic = istart, ncell, istep
      ...
      loop_over_species: do is=1, n_thermal !This loop has to come first
          loop_over_dcx: do idcx=1, nlaunch(i,j,k) !! Loop over the markers
              ...
              ! Sample ion GC distribution function:
              ! input: ind, is
              ! output: rgc, deni_gc, fields, rp, vp, ind_p
              call mc_sample_ion_f4d_gc(ind,is,rgc,deni_gc,fields,rp,vp,ind_p,denf4d,f4d_defined)
              if(dot_product(vp,vp).eq.0.) then
                write (*,*) "vp .eq. 0 skipping marker ..."
                write (*,*) "vp:", vp
                write (*,*) "ind: ", ind
                write (*,*) "rp: ", rp
                cycle loop_over_dcx
              ...
              states = deni_gc*rates !  flux per unit volume [p/s cm^-3]
              ...
              call store_sink_particle(ind_p,rp,vp,is,flux_per_marker,denf4d,denf4d_per_marker,fields)
              ...
              if ( .FALSE. .AND. (.not. absorbed) .AND. (final_flux/starting_flux .gt. 1E-2) ) then
...
subroutine calculate_halo_process
  ...
  iterations: do hh = 1,n_iter
    ...
    !$OMP PARALLEL DO schedule(dynamic,1) private(ic,is,ihalo,absorbed,ind,i,j,k,ri,vi,denf4d,f4d_defined,tracks, &
    ...
    loop_over_cells: do ic = istart,ncell,istep
      ...
      loop_over_species: do is = 1,n_thermal
        loop_over_halos: do ihalo = 1,nlaunch(i,j,k)
          ...
          !! Pick ion from GC distribution function:
          ...
          absorbed = .FALSE.
          call track_to_wall(ri,vi,tracks,ntrack,absorbed)
          if (ntrack .eq. 0) then
...
subroutine mc_ion_loss_rate(plasma,is,ab,vn,H_H_depop)
  ...
  loop_over_ions: do ii = 1, num_ions
      ...
      call mc_sample_ion_f4d_gc(ind,is,rgc,deni_gc,fields_gc,ri,vi,ind_p,denf4d,f4d_defined)
